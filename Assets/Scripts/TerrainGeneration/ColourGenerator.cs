...
public class ColourGenerator {
    ...
    public void UpdateSettings(ColourSettings settings) {
        ...
        biomeNoiseFliter = NoiseFilterFactory.CreateNoiseFilter(settings.biomeColourSettings.noise);
    }
    ...
    public float BiomePercentFormPoint(Vector3 pointOnUnitSphere) {
        float heightPercent = (pointOnUnitSphere.y + 1) / 2f;
        heightPercent += (biomeNoiseFliter.Evaluate(pointOnUnitSphere) - settings.biomeColourSettings.noiseOffset) * settings.biomeColourSettings.noiseStrength;
        float biomeIndex = 0;
    ...
    public void UpdateColours() {
        ...
        foreach (var biome in settings.biomeColourSettings.biomes) {
            for (int i = 0; i < textureResolution * 2; i++) {
                ...
                colours[colourIndex] = gradientCol * (1 - biome.tintPrecent) + tintCol * biome.tintPrecent;
                colourIndex++;
            }
        }
